...
public class IncidenciaDAO {
    ...
    public void insertar(Incidencia i) {
        // 🔒 No insertar si descripción es nula o vacía (para que el total no cambie)
        if (i.getDescripcion() == null || i.getDescripcion().trim().isEmpty()) {
            return;
        }
        // Default de estado
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            cs.setString(1, i.getDescripcion().trim());
            ...
            insertarSinSP(i.getDescripcion(), estado, i.getIdUsuario(), i.getIdEquipo(), i.getIdTecnico());
        }
    }

    // Insertar normal (fallback)
    private void insertarSinSP(String descripcion, String estado, int idUsuario, int idEquipo, Integer idTecnico) {
        // Ya validado y normalizado arriba
    ...
    public void actualizar(Incidencia i) {
        // Evitar estado nulo/vacío al actualizar
        String estado = i.getEstado();
        if (estado == null || estado.trim().isEmpty()) {
            estado = "Abierta";
        }
        ...
        try (Connection conn = DatabaseConnection.getConnection();
            ...
            cs.setString(2, (i.getDescripcion() == null ? "" : i.getDescripcion().trim()));
    ...
    private Incidencia mapearIncidencia(ResultSet rs) throws SQLException {
        ...
        // Datos relacionados
        // OJO: estos aliases pueden ser null si no están en el SELECT actual
        try { i.setNombreUsuario(rs.getString("nombreUsuario")); } catch (SQLException ignored) {}
        try { i.setCodigoEquipo(rs.getString("codigoEquipo")); } catch (SQLException ignored) {}
        try { i.setTipoEquipo(rs.getString("tipoEquipo")); } catch (SQLException ignored) {}
        try { i.setNombreTecnico(rs.getString("nombreTecnico")); } catch (SQLException ignored) {}
    ...
    private static final String BASE_QUERY =
        ...
        "e.idEquipo AS codigoEquipo, " +   // (nota: aquí devuelves idEquipo como 'codigoEquipo')
